...
export class DroneService {
    ...
    async loadItem(droneId: string, medicamentId: string) {
        ...
        if (drone.batteryCapacity < 25) {
            throw new BadRequestException('Drone can\'t be loaded when the battery is below 25%')
        }
        ...
        if (loadedWeight + medicament.weight > drone.weightLimit) {
            throw new BadRequestException('Weight limit of the drone has been reached')
        }
        ...
        if (!loadedDrone) {
            throw new HttpException('Underlying storage was modified while executing change')
        }

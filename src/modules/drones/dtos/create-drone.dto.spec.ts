...
const invalidDtos: {
...
}[] = [
  {
    ...
    message: {
      'message': [{ 'maxLength': 'serialNumber must be shorter than or equal to 100 characters' }],
      'status': 400
    }
  },
  {
    ...
    message: {
      'message': [{ 'isEnum': 'model must be one of the following values: Lightweight, Middleweight, Cruiserweight, Heavyweight' }],
      'status': 400
    }
  },
  {
  ...
  },
  {
    ...
    message: {
      'message': [{ 'isEnum': 'state must be one of the following values: IDLE, LOADING, LOADED, DELIVERING, DELIVERED, RETURNING' }],
      'status': 400
    }
  }
]
...
for (const { label, dto, message } of invalidDtos) {
  it(`${label}`, () =>
    expect(validateInput(CreateDroneDto, { ...createDroneDto, ...dto }))
      .rejects
      .toEqual(message)
  )
}
